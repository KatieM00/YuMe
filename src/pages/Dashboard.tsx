...
const DashboardContent = ({ setPage }: DashboardProps) => {
  ...
  const ButtonBox = ({ id, name, iconSrc }: typeof navButtons[0]) => (
    <button
        onClick={() => setPage(id)}
        className="relative group transition-transform duration-300 hover:scale-105 will-change-transform"
    >
      <img src={iconSrc} alt={name} className="w-16 h-16 sm:w-20 sm:h-20 md:w-24 md:h-24 lg:w-28 lg:h-28 pointer-events-none" />
      <div className="absolute inset-0 bg-white/[0.03] rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-700 ease-in-out flex items-center justify-center pointer-events-none will-change-opacity">
        <span className="text-white text-sm sm:text-base font-semibold drop-shadow-[0_2px_8px_rgba(0,0,0,0.8)] pointer-events-none">{name}</span>
      </div>
    </button>
  );
